...
\newcommand{\Afterburn}{\includegraphics[height = .4cm]{afterburn}}
...
\newcommand{\cardcontent}[1]{
    %\node[text width=4.5cm] at (\cardwidth/2, 0.35*\cardheight) {#1};
		\node[text width=4.75cm, below right] at (\stripwidth*1.02, 0.535*\cardheight) {\textrm{#1}};
}
...
\newcommand{\myburn}[1]{
	\node[text width=7.5cm, rotate=-90] at (\cardwidth*.92, \cardheight*.46) {\Afterburn \tiny{#1}};
}
