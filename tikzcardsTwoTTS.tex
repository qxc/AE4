...
\newcommand{\Money}{\includegraphics[height = 1cm]{money}}
...
\newcommand{\Health}{\includegraphics[height = 1.2cm]{heart}}
\newcommand{\Shield}{\includegraphics[height = 1.2cm]{shield}}
...
\newcommand{\cardcontent}[1]{
    %\node[text width=4.5cm] at (\cardwidth/2, 0.35*\cardheight) {#1};
		\node[text width=.73*\cardwidth cm, below right] at (.15*\cardwidth, 0.48*\cardheight) {\Huge{#1}};
}

\newcommand{\tier}[1]{
    %\node[text width=4.5cm] at (\cardwidth/2, 0.35*\cardheight) {#1};
	\node at (.2*\cardwidth, .9*\cardheight) {\Huge{#1}};
}
...
\newcommand{\cardhp}[1]{
		\node[left] at (\cardwidth*.88,\cardheight*.83) {\textbf{\Huge #1}\Health};
}
\newcommand{\cardshield}[1]{
		\node[left] at (\cardwidth*.88,\cardheight*.72) {\textbf{\Huge #1} \Shield};
}
