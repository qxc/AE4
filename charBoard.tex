...
\newcommand{\basicinfo}{
	...
	\node at (-8, -5) {\LARGE{Life:}};
	\draw (-6,-6) rectangle (-6+\squarewidth*6,-6+\squareheight);
}

\newcommand{\notes}[1]{
	\node[text width=7cm, left] at (\squarewidth*.05 - 2, \squareheight*.1 - 2) {\textbf{Notes:} #1};
}

\newcommand{\name}[1]{
\node at (0, 3) {\LARGE{#1}};
\draw (-\squarewidth/2,2.5) rectangle (\squarewidth/2, 2.5-\squareheight);
\node[text width = 2 cm] at (0, 2.5-\squareheight/2) {\begin{center}Player \\ Number\end{center}};
}
...
\node at (-9+\squarewidth/2, -9.75+\squareheight/2) {\includegraphics[height = 2cm]{chargetest}};
\fi

\ifnum 2>#2\relax\else
\node at (-6+\squarewidth/2, -9.75+\squareheight/2) {\includegraphics[height = 2cm]{chargetest}};
\fi

\ifnum 3>#2\relax\else
\node at (-3+\squarewidth/2, -9.75+\squareheight/2) {\includegraphics[height = 2cm]{chargetest}};
\fi

\ifnum 4>#2\relax\else
\node at (0+\squarewidth/2, -9.75+\squareheight/2) {\includegraphics[height = 2cm]{chargetest}};
\fi
